#########################
library(ranger)
if (!require("crossRanger", quietly = TRUE)) {
  devtools::install_github('shihuang047/crossRanger',repos = "http://cran.us.r-project.org")
  library("crossRanger")
} else {
  library("crossRanger")
}
...
save_feature_importance <- function(feature_importance, outpath_dir){
  ...
  feature_importance <- select(feature_importance, "features", everything())
  write.table(feature_importance, outpath_dir, sep="\t", row.names = F, quote = F, col.names = T)
}

#predicted skin age in the new dataset
